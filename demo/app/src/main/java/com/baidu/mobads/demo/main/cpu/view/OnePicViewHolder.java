...
public class OnePicViewHolder extends AbstractViewHolder {
    ...
    public OnePicViewHolder(View view) {
        super(view);
        container = view.findViewById(R.id.top_text_view);
        imageView = view.findViewById(R.id.image_cc);
    }
    ...
    public void initWidgetWithData(final IBasicCPUData nrAd, int position) {
        ...
        if (nrAd.getType().equals("ad")) {
            Glide.with(mCtx).load(imageList.get(0)).into(imageView);
        } else {
            Glide.with(mCtx).load(smallImageList.get(0)).into(imageView);
        }
