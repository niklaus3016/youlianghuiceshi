...
public class NativeExpressDemoActivity extends BaseActivity implements View.OnClickListener,
  ...
  protected EditText editTextWidth, editTextHeight; // 编辑框输入的宽高
  private int adWidth, adHeight; // 广告宽高
  protected CheckBox checkBoxFullWidth, checkBoxAutoHeight;
  private boolean isAdFullWidth, isAdAutoHeight; // 是否采用了ADSize.FULL_WIDTH，ADSize.AUTO_HEIGHT
  private boolean isPreloadVideo;
  private boolean mLoadSuccess;

  protected int viewHeight = 320;
  protected int viewWidth = 340;
  ...
  private int getAdCount() {
    int defaultValue = 1;
    int intExtra = getIntent().getIntExtra(Constants.LOAD_AD_COUNT, defaultValue);
    if (intExtra <= 0) {
      intExtra = defaultValue;
    }
    return intExtra;
  }
  ...
  private void refreshAd() {
    try {
      ...
      nativeExpressAD.loadAD(getAdCount(), DemoUtil.getLoadAdParams("native_express"));
  ...
  protected void resizeAd() {
    try {
      ...
      if (checkEditTextChanged()) {
        ...
        layoutParams.width = isAdFullWidth ? ViewGroup.LayoutParams.MATCH_PARENT : PxUtils.dpToPx(this, adWidth);
        layoutParams.height = isAdAutoHeight ? ViewGroup.LayoutParams.WRAP_CONTENT : PxUtils.dpToPx(this, adHeight);
  ...
  public void onNoAD(AdError adError) {
    Log.i(
        ...
        String.format("onNoAD, error code: %d, error msg: %s", adError.getErrorCode(),
            adError.getErrorMsg()));
    BuildConfig.CallbackLogUtil.logNoAd(TAG);
    DemoBiddingC2SUtils.reportBiddingNoAd(nativeExpressAD); // 仅支持上报无广告
  }
  ...
  public void onADLoaded(List<NativeExpressADView> adList) {
    ...
    BuildConfig.CallbackLogUtil.logAdLoaded(TAG);
...
//    nativeExpressADView.sendLossNotification(1, 1, null);
    ...
    // 设置激励回调监听
    nativeExpressAD.setRewardListener(this);
    ServerSideVerificationOptions options = new ServerSideVerificationOptions.Builder()
        .setCustomData("APP's custom data") // 设置服务端验证的自定义信息
        .setUserId("APP's user id for server verify") // 设置服务端验证的用户信息
        .build();
    nativeExpressAD.setServerSideVerificationOptions(options);
    Log.i(TAG, "onADLoaded, video info: " + getAdInfo(nativeExpressADView) + ", getAdNetWorkName: " +
        nativeExpressAD.getAdNetWorkName());
    AdData boundData = nativeExpressADView.getBoundData();
    if (boundData != null && boundData.getAdPatternType() == AdPatternType.NATIVE_VIDEO) {
   ...
   */
  private void reportBiddingResult(IBidding adData) {
  ...
  public void onRenderFail(NativeExpressADView adView) {
    BuildConfig.CallbackLogUtil.logRenderFail(TAG);
  }

  @Override
  public void onRenderSuccess(NativeExpressADView adView) {
    BuildConfig.CallbackLogUtil.logRenderSuccess(TAG);
  }

  @Override
  public void onADExposure(NativeExpressADView adView) {
    Log.i(TAG, "onADExposure traceid=" + adView.getExtraInfo().get(REQUEST_ID));
    BuildConfig.CallbackLogUtil.logAdExpose(TAG);
  }

  @Override
  public void onADClicked(NativeExpressADView adView) {
    Log.i(TAG, "onADClicked traceid=" + adView.getExtraInfo().get(REQUEST_ID));
    BuildConfig.CallbackLogUtil.logAdClick(TAG);
  }

  @Override
  public void onADClosed(NativeExpressADView adView) {
    BuildConfig.CallbackLogUtil.logAdClose(TAG);
    // 当广告模板中的关闭按钮被点击时，广告将不再展示。NativeExpressADView也会被Destroy，释放资源，不可以再用来展示。
    if (container != null && container.getChildCount() > 0) {
      container.removeAllViews();
      container.setVisibility(View.GONE);
    }
    adView.destroy(); // 及时销毁广告
  }

  @Override
  public void onADLeftApplication(NativeExpressADView adView) {
    Log.i(TAG, "onADLeftApplication");
    BuildConfig.CallbackLogUtil.logADLeftApplication(TAG);
  }

  @Override
  public void onReward(Map<String, Object> map) {
    NativeExpressADView adView = (NativeExpressADView) map.get(IReward.AD_VIEW);
    Log.i(TAG, "onReward transId=" + map.get(ServerSideVerificationOptions.TRANS_ID)
        + ", devExt=" + map.get(IReward.DEV_EXT)
        + ", adView=" + adView);
  }
  ...
  public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
    if (buttonView.getId() == R.id.checkboxFullWidth) {
      if (isChecked) {
        ...
        isAdFullWidth = false;
        editTextWidth.setText(String.valueOf(viewWidth));
    ...
    if (buttonView.getId() == R.id.checkboxAutoHeight) {
      if (isChecked) {
        ...
        editTextHeight.setText(String.valueOf(viewHeight));
