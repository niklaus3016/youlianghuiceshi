...
public class NativeADUnifiedSampleActivity extends BaseActivity implements NativeADUnifiedListener {
    ...
    if (ads != null && ads.size() > 0) {
      ...
      if (miitInfo != null) {
        miitStr = "miit info appName ='" + miitInfo.getAppName() + '\'' +
            ...
            ", icpInfoUrl='" + miitInfo.getIcpInfoUrl() + '\'' +
            '}';
      } else {
  ...
  protected void loadAd() {
    ...
    if(mAdManager != null) {
      LoadAdParams loadAdParams = DemoUtil.getLoadAdParams("native_unified");
      mAdManager.loadData(AD_COUNT, loadAdParams);
    }
  }
  ...
  protected void onDestroy() {
    super.onDestroy();
    if (mAdData != null) {
      // 必须要在Activity.destroy()时销毁广告数据，释放内存
      mAdData.destroy();
    }
  }
  ...
  public void onNoAD(AdError error) {
    ...
    DemoBiddingC2SUtils.reportBiddingNoAd(mAdManager); // 仅支持上报无广告
  }
  ...
  protected void layoutWithOrientation() {
    mCustomContainer = findViewById(R.id.custom_container);
