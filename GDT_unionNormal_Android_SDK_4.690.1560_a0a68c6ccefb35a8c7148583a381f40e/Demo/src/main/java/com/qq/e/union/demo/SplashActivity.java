...
import com.qq.e.union.demo.util.CallbackLogUtil;
...
public class SplashActivity extends BaseActivity implements SplashADListener,
  ...
  private boolean isClearWindowFullScreenFlag = false;
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    if (isClearWindowFullScreenFlag) {
    ...
    mLogoPortrait = findViewById(R.id.app_logo_vertical);
    mLogoLandscape = findViewById(R.id.app_logo_landscape);
    // ！！！注意：在半屏情况下，【竖屏和横屏】高度要求，是不一样的！！！
    // 【竖屏广告】要求广告容器高度不小于屏幕高度的80%，【横屏广告】要求广告容器高度不小于屏幕高度（是竖屏的宽）的95%
    // 所以预留给开发者展示logo等信息的区域，需要注意大小合适
    if (this.getResources().getConfiguration().orientation == Configuration.ORIENTATION_LANDSCAPE) {
      mLogoPortrait.setVisibility(View.GONE);
      mLogoLandscape.setVisibility(View.VISIBLE);
    } else if (this.getResources().getConfiguration().orientation == Configuration.ORIENTATION_PORTRAIT) {
      mLogoLandscape.setVisibility(View.GONE);
    ...
    if (!needLogo) {
      findViewById(R.id.app_logo_container).setVisibility(View.GONE);
    }
...
//    }
  ...
  public void onADPresent() {
    BuildConfig.CallbackLogUtil.logAdPresent(TAG);
  }
  ...
  public void onADTick(long millisUntilFinished) {
    ...
    BuildConfig.CallbackLogUtil.logAdTick(TAG);
  }
  ...
  public void onNoAD(AdError error) {
    String str = String.format("LoadSplashADFail, eCode=%d, errorMsg=%s", error.getErrorCode(),
        error.getErrorMsg());
    DemoBiddingC2SUtils.reportBiddingNoAd(splashAD);
    Log.i("AD_DEMO",str);
  ...
  private void getSplashAdSettings() {
    ...
    isClearWindowFullScreenFlag = sp.getBoolean("splashClearWindowFullScreenFlag", false);
  }
}
